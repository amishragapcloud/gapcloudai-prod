...
# Sidebar for alert creation, viewing, editing, and email notification
with st.sidebar:
    st.title("Alert Configuration")
    if schemas:  # Check if schemas is not None
        table_options = list(schemas.keys())  # Get the table names from schemas
        selected_table = st.selectbox("Select Table", table_options)
        if selected_table:
            column_options = schemas[selected_table]  # Get the column names for the selected table
            selected_column = st.selectbox("Select Column", column_options)
    else:
        st.error("Failed to retrieve schema information. Check debug logs.")

    # Add the rest of your sidebar code here...

    condition = st.selectbox("Condition", ['Greater Than', 'Less Than'])
    value = st.number_input("Enter Value", min_value=0.0, max_value=100.0, step=0.1)
...
st.markdown('<span style="color: orange;; display: inline;">Don\'t know where to start?</span>''<span style="color: green;; display: inline;"> Try These.....</span>', unsafe_allow_html=True)
col1, col2, col3, col4 = st.columns(4)
with col1:
    if st.button("Show me the Avg ACW Time last week."):
...
# Add a text input box for the email address
email_address = st.text_input("Enter your email address:")
